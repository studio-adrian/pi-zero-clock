...
epd_GRAY1  = 0xff #white
epd_GRAY2  = 0xC0
epd_GRAY3  = 0x80 #gray
epd_GRAY4  = 0x00 #Blackest
...
def draw_axis(draw):
    draw_arrow(draw, 25,25, 0, render_width / 4, epd_GRAY4)
    draw_arrow(draw, 25,25, 90, render_height / 4, epd_GRAY2)
...
def draw_day_clock(draw, current_time, center_x, center_y, size):
    ...
    # draw the clock face
    draw.ellipse((center_x-face_radius, center_y-face_radius, center_x+face_radius, center_y+face_radius), fill=epd_GRAY1)
    draw.ellipse((center_x-face_radius, center_y-face_radius, center_x+face_radius, center_y+face_radius), outline=epd_GRAY4, width=line_width)
    # draw the clock center
    draw.ellipse((center_x-center_radius, center_y-center_radius, center_x+center_radius, center_y+center_radius), fill=epd_GRAY4)
    ...
    draw.line((center_x, center_y, hour_x, hour_y), fill=epd_GRAY4, width=line_width*2)
    ...
    draw.line((center_x, center_y, minute_x, minute_y), fill=epd_GRAY4, width=line_width*2)
    ...
    #draw.line((center_x, center_y, second_x, second_y), fill=epd_GRAY4, width=line_width)
    ...
    if bDrawNumbers:
        ...
        for i in range(1,13):
            ...
            draw.line((tick_x1, tick_y1, tick_x2, tick_y2), fill=epd_GRAY4, width=int(6*1.5))
            # draw the hour
            ...
            text_width, text_height = draw.textsize(str(i), font=font35)
            hour_x -= text_width / 2
            hour_y -= text_height / 2
            draw.text((hour_x, hour_y), str(i), font=font35, fill=epd_GRAY4)
    ...
    for i in range(60):
        ...
        tick_x2 = center_x + face_radius * math.cos(math.radians(angle))
        tick_y2 = center_y + face_radius * math.sin(math.radians(angle))
        draw.line((tick_x1, tick_y1, tick_x2, tick_y2), fill=epd_GRAY4, width=6)
...
def draw_year_clock(draw, current_time):
    ...
    for i in range(365):
        ...
        tick_x2 = center_x + face_radius * math.cos(math.radians(angle))
        tick_y2 = center_y + face_radius * math.sin(math.radians(angle))
        draw.line((tick_x1, tick_y1, tick_x2, tick_y2), fill=epd_GRAY4, width=3)
    ...
    for i in range(12):
        ...
        #draw.ellipse((month_x-10, month_y-10, month_x+10, month_y+10), outline=epd_GRAY4, width=3)
        # get the text bounds for the month name
        text_width, text_height = draw.textsize(months[i], font=font35)
        month_x -= text_width / 2
        month_y -= text_height / 2
        draw.text((month_x, month_y), months[i], font=font35, fill=epd_GRAY4)
        nDays += days[i]
...
def draw_year_clock_with_weeks(draw, current_time, center_x, center_y, size):
    ...
    draw.line((center_x, center_y, hour_x, hour_y), fill=epd_GRAY4, width=int(line_width*0.5))
    ...
    draw.line((center_x, center_y, hour_x, hour_y), fill=epd_GRAY4, width=line_width*3)
    
    
    # draw a arc from the start of the year to the current day
    draw.arc((center_x-face_radius, center_y-face_radius, center_x+face_radius, center_y+face_radius), -90, hour_angle, fill=epd_GRAY2, width=120)
    ...
    for i, sprint in enumerate(sprints):
        ...
        tick_x2 = center_x + face_radius * math.cos(math.radians(angle))
        tick_y2 = center_y + face_radius * math.sin(math.radians(angle))
        draw.line((tick_x1, tick_y1, tick_x2, tick_y2), fill=epd_GRAY4, width=12)
        ...
        #draw.ellipse((month_x-10, month_y-10, month_x+10, month_y+10), outline=epd_GRAY4, width=3)
        # get the text bounds for the month name
        text_width, text_height = draw.textsize(str(sprint_name), font=font24)
        sprint_x -= text_width / 2
        sprint_y -= text_height / 2
        draw.text((sprint_x, sprint_y), str(sprint_name), font=font24, fill=epd_GRAY4)
        nDays += sprint*7
...
def draw_background(draw):
    for _ in range(1000): 
        ...
        draw_flower(draw, x, y, size, petals, epd_GRAY2)
    ...
    for _ in range(50): 
        ...
        draw_swirl(draw, x, y, size, turns, 1, epd_GRAY2)
...
def render(draw):
    ...
    # Display the image
    resized_image = image.resize((epd_width, epd_height), resample=Image.LANCZOS)
    resized_image.show()    
...
if __name__ == "__main__":
    try:
        ...
        image = Image.new('L', (render_width, render_height), epd_GRAY1)  # clear the frame
