...
def get_current_date_string():
    ...
    date_string = now.strftime("%A, %B %d")  # Example: "Monday, October 23"
    return date_string
...
def draw_day_clock(draw, current_time, center_x, center_y, size):
    ...
    # blank out the face
    draw.ellipse((center_x-face_radius*1.05, center_y-face_radius*1.05, center_x+face_radius*1.05, center_y+face_radius*1.05), fill=epd.GRAY1)
    ...
    if bDrawNumbers:
        ...
        for i in range(1,13):
            ...
            number_radius = int(face_radius * 0.15)
            draw.ellipse((hour_x-number_radius, hour_y-number_radius, hour_x+number_radius, hour_y+number_radius), fill=epd.GRAY1) #, outline=epd.GRAY4, width=1) # fill=epd.GRAY1)
...
def draw_year_clock_with_weeks(draw, current_time, center_x, center_y, size):
    ...
    if bDrawMonths:
        ...
        for i in range(12):
            ...
            number_radius = int(face_radius * 0.20)
            draw.ellipse((month_x-number_radius, month_y-number_radius, month_x+number_radius, month_y+number_radius), fill=epd.GRAY1) #, outline=epd.GRAY4, width=1) # , fill=epd.GRAY1)
            nDays += days[i]
...
def draw_background(draw):
    for _ in range(1000):
        y = random.randint(0, render_width)
        x = random.randint(0, render_height)
        size = random.randint(10, 45)
        petals = random.randint(7, 12)
        draw_flower(draw, x, y, size, petals, epd.GRAY4)
    ...
    for _ in range(50):
        ...
        draw_swirl(draw, x, y, size, turns, 1, epd.GRAY4)


def draw_frame( draw ):
    draw.text((render_height*0.6, render_width - 50), 'Year Clock - 2025 - Adrian Ferrier', font = font24, fill = 0)

    # Draw the background
    draw_background(draw)

    # Draw the clock face
    center_y = int(render_width / 4 * 3)
    center_x = int(render_height / 2)
    size = render_height / 2 * 0.8 * 0.75

    draw_year_clock_with_weeks(draw, time.localtime(), center_x, center_y, size)

    center_y = int(render_width / 4 * 1)
    draw_day_clock(draw, time.localtime(), center_x, center_y, size)

    # draw day and date
    message = get_current_date_string()
    text_width, text_height = draw_textsize(message, font=font35)
    draw.text((render_height/2.0 - text_width/2, render_width/2*0.7 - text_height/2), message, font=font35, fill=epd.GRAY4)
...
try:
    ...
    factor = 4
    render_width, render_height = epd.width*factor, epd.height*factor
    ...
    Himage = Image.new('L', (render_height, render_width), 255)  # 255: clear the frame
    draw = ImageDraw.Draw(Himage)

    while (True):
        draw.rectangle((0, 0, render_height, render_width), fill = 255)

        draw_frame(draw)

        resized_image = Himage.resize((epd.height, epd.width), resample=Image.LANCZOS)
        #rotated_image = resized_image.rotate(180)
        if ( g_bGrayscaleMode ):
            epd.display_4Gray(epd.getbuffer_4Gray(resized_image))
        else:
            epd.display_Partial(epd.getbuffer(resized_image))
