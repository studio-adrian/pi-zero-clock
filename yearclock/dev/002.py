...
#render_width = epd.width
#render_height = epd.height
...
def render():
    ...
    # Draw the clock face
    center_x = int(epd.width / 4 * 3)
    center_y = int(epd.height / 2)
    size = epd.height / 2 * 0.8 * 0.75
    draw_year_clock_with_weeks(draw, time.localtime(), center_x, center_y, size)
    
    center_x = int(epd.width / 4 * 1)
...
#    resized_image = image.resize((epd.width, epd.height), resample=Image.LANCZOS)
    #resized_image.show()    
    epd.display_Fast(epd.getbuffer_4Gray(image))
...
def run_every_minute():
    while True:
        ...
        next_minute = now + datetime.timedelta(seconds=10)
