...
def draw_frame( draw ):
    draw.text((render_width * 0.75, render_height - 50), 'Year Clock - 2025 - Adrian Ferrier', font = font24, fill = 0)
...
try:
    ...
    Himage = Image.new('L', (render_width, render_height), 255)  # 255: clear the frame
    draw = ImageDraw.Draw(Himage)

    draw_frame(draw)

    # Finalize the image
    resized_image = Himage.resize((epd.width, epd.height), resample=Image.LANCZOS)
    if ( g_bGrayscaleMode ):
        epd.display_4Gray(epd.getbuffer_4Gray(resized_image))
    else:
        epd.display_Base(epd.getbuffer(resized_image))

    time.sleep(2)

    while (True):
        draw.rectangle((0, 0, render_width, render_height), fill = 255)
        ...
        if ( g_bGrayscaleMode ):
            ...
            epd.display_Partial(epd.getbuffer(rotated_image))
